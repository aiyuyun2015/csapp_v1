...
void eval(char *cmdline)
{
    ...
    // If your shell then creates a child process, by default that child will also be a member of the foreground process
    // group.
    // Since typing ctrl-c sends a SIGINT to every process in the foreground group,
    // typing ctrl-c will send a SIGINT to your shell, as well as to every process that your shell created, which
    ...
    // When you type ctrl-c, the shell should catch the resulting SIGINT and then forward it to the appropriate
    // foreground job
    // (or more precisely, the process group that contains the foreground job).



    // The parent needs to block the SIGCHLD signals in this way in order to avoid the race condition where the child
    // is reaped by sigchld handler (and thus removed from the job list) before the parent calls addjob.
    /* ########################### TO DO ########################### */
    ...
    // if its' built in, do inside builtin_cmd, otherwise, do it here
    if (!builtin_cmd(argv)) {
        // Blocking SIGCHILD
...
void do_bgfg(char **argv)
{
    ...
    if(strcmp(argv[0], "bg") == 0) {
        ...
        // must be fg, even though no checking
...
void sigchld_handler(int sig)
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {    // Reap a zombie child
        ...
        // For example, SIGINT default behavior is terminate
            ...
            deletejob(jobs,pid);
            if (verbose) printf("sigchld_handler: Job [%d] (%d) deleted\n", jobid, (int) pid);
            // WTERMSIG is used to find the signal number
            printf("Job [%d] (%d) terminated by signal %d\n", jobid, (int) pid, WTERMSIG(status));
        }

        // WIFSTOPPED returns true if the child that cause the return is currently stopped.
        // ctrl+z is also a termination, will trigger SIGCHLD, however, this is handled by SIFSTOPPED
...
void sigint_handler(int sig)
{
    // be sure to send SIGINT and SIGTSTP signals to the entire foreground process group, using ”-pid” instead of ”pid”
    // in the argument to the kill function. The sdriver.pl program tests for this error.
    /* ########################### TO DO ########################### */
    if (verbose) printf("sigint_handler: entering\n");

    // find all the current jobs, fg jobs?
...
void sigtstp_handler(int sig)
{
    // be sure to send SIGINT and SIGTSTP signals to the en- tire foreground process group,
    // using ”-pid” instead of ”pid” in the argument to the kill function. The sdriver.pl program tests for this error.
    /* ########################### TO DO ########################### */
